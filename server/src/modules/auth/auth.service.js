...
import {generateResetToken, generateAccessToken,
    generateRefreshToken,verifyRefreshToken} from '../../common/utils/jwt.utils.js'
...
const register = async ({ name, email, password, role }) => {
    ...
    const user = await User.create({
        name, email, password, role, verificationToken: hashedToken
    })
    ...
    try {
        await sendEmail({
            ...
            text: `Welcome ${name}! Please verify your email using this token: ${rawToken}`,
            html: `<h1>Email Verification</h1><p>Welcome ${name}!</p><p>Please verify your email using this token: <strong>${rawToken}</strong></p>`
        });
    } catch (error) {
...
const login = async ({ email, password }) => {
    ...
    if (!user.isVerified) {
        throw ApiError.forbidden("Please Verify your Email Address.")
    }
...
export { register, login, logout, refresh, forgotPassword, resetPassword,getMe,verifyUser, greetHello}